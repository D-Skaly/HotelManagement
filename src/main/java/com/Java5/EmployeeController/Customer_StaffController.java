...
public class Customer_StaffController {

    @Autowired
    CustomerService customerService;

    // Hiển thị danh sách khách hàng với phân trang và sắp xếp
    @GetMapping("/customer")
    public String show(Model model, @RequestParam(defaultValue = "0") int page,
                       @RequestParam(defaultValue = "") String keyword,
                       @RequestParam(defaultValue = "userName") String sortBy,
                       @RequestParam(defaultValue = "asc") String sortOrder,
                       @RequestParam("activeTab") Optional<String> tab) {
    ...
    @GetMapping("customermanager/edit/{userName}")
    ...
    @PostMapping("/customermanager/update")
